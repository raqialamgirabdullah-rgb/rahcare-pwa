...
window.onTypeChange = function (box, type) {
  let other = g(type === "Home" ? "typeCenter" : "typeHome");
  if (!box.checked) {
    if (!other.checked) box.checked = true;
  } else {
    other.checked = false;
  }
  updateSelectedType();
...
function updateSelectedType() {
  window.selectedType = g("typeHome").checked ? "Home" : "Center";
}
...
function showDistricts(division) {
  ...
  addressData[division].forEach(district => {
    ...
    span.onclick = () => {
      g("addrVal").value = `${currentAddress}, ${district}`;
...
window.pickPatientType = function (val) {
  g("ptypeVal").textContent = val;
  g("ptypeMenu").classList.remove("open");
};
...
window.onAddrTyped = function (input) {
  if (input.value.trim()) {
    g("addrBtn").classList.add("selected");
  } else {
...
window.clearForm = function () {
  ...
  window.selectedCategory = "Ruqyah";
  g("typeHome").checked = false;
  g("typeCenter").checked = true;
  window.selectedType = "Center";
  g("ptypeVal").textContent = "New";
  buildAddressMenu();
};
...
window.generateSlip = async function () {
  ...
  let type = window.selectedType || "Center";
  let patientType = g("ptypeVal").textContent || "New";
  ...
  function buildAppointment(cat) {
    return {
      uid, date, idNumber, name, phone, gender, age, address, time, category: cat, type, patientType,
      bill: "Unpaid",
      serviceName: "--",
      serviceCharge: "--",
      serviceQuantity: "--",
  ...
  let categoriesToSave = category === "Ruqyah + Hijama" ? ["Ruqyah", "Hijama"] : [category];

  try {
    await Promise.all(categoriesToSave.map(cat => addDoc(collection(db, "appointments"), buildAppointment(cat))));
